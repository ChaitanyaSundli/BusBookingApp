...
class TripInfoBottomSheet extends StatelessWidget {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('${trip.operator?.companyName ?? ''} • ${trip.bus?.busName ?? ''}',
          ...
          Row(
            children: [
              ...
              const Spacer(),
              Text('Total: ₹${totalFare.toStringAsFixed(2)}', style: const TextStyle(fontWeight: FontWeight.bold)),
            ],
          ),
  ...
  Widget _buildSection({required String title, required IconData icon, required Widget child}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(children: [Icon(icon, size: 18, color: Colors.grey.shade700), const SizedBox(width: 8), Text(title, style: const TextStyle(fontWeight: FontWeight.w600))]),
        const SizedBox(height: 8),
        child,
      ],
    );
  ...
  Widget _buildPolicyRow(String condition, String refund) {
    return Padding(
      ...
      child: Row(children: [SizedBox(width: 100, child: Text(condition)), Text(refund, style: const TextStyle(fontWeight: FontWeight.w500))]),
    );
  }

  Widget _buildAmenity(IconData icon, String label) {
    return Row(mainAxisSize: MainAxisSize.min, children: [Icon(icon, size: 16, color: Colors.grey.shade600), const SizedBox(width: 4), Text(label)]);
  }
}